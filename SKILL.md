---
name: write-compact-wechat-ai
description: 面向毫无 AI 背景的产品经理，撰写或改写约 1000 字、简短直接、段落紧凑的微信公众号 AI 技术名词解释。正文用自然中文解释概念，案例采用小号灰色等明显区分的样式，资料名称在正文用书名号或括号标注，完整链接统一列在文末。仅在微信公众号 AI 入门科普、AI 术语解释和同类短文任务中使用。
---
...
本节是当前 Skill 对下文默认写法的唯一覆盖。遇到冲突时先执行本节，未提到的事实边界、创作流程、中文表达、禁用项和审稿规则全部保持不变。

- 默认写给毫无 AI 背景的产品经理。第一次出现技术名词时写出英文全称和中文名称，随后立即用一句普通话解释。读者理解当前概念以后就往下走，不继续展开无关的术语、发展史和研究分支。能不引入第二个技术名词就不引入；确实需要时只用一句白话解释，不能另开一节继续扩展。
- 正文默认控制在八百至一千一百个汉字，接近一千字即可，参考资料不计入。材料能讲清时可以更短，不能为了凑字数重复解释。初稿超过一千一百字，先删旁支概念、技术实现细节和历史背景，再检查是否仍然完整。用户明确指定其他篇幅时服从用户。
- 文章保持简短、直接。一个主要概念配一个贴近日常或产品工作的案例，讲清定义、用途和使用时最需要注意的一件事。最多补充一个确实能帮助初学者的概念区别，不能为了显得完整自动加入相邻技术、训练方式或评估体系。
- 段落采用微信公众号的紧凑写法。同一部分的几句话放在同一段里，每段集中处理一个问题；一句通常只承担一个主要意思，逗号不过多。不要一句话单独占一段，也不要在每个句号后换行。内容切换时再另起一段。
- 小标题只用于区分确有不同的部分，通常保留两至三个。小标题和正文之间不堆空行，不把文章切成许多碎片。
- 案例必须与正文一眼区分。交付富文本或 HTML 时，正文默认十六像素，案例默认十四像素灰色文字，可配浅灰背景和细左边线；使用内联样式，便于从浏览器复制进微信公众号。只交 Markdown 或纯文本时，把案例放进引用块并标注“案例”。
- 正文不把“论文原文”“官方建议”等字样做成隐藏链接。正文提到资料时，用中文书名号或圆括号标出资料名称。完整来源统一放在文末“参考资料”，同时写出标题和可见的原始网址，避免粘贴到公众号后丢失链接地址。
- 用户要求公众号排版时，优先交付可在浏览器打开并复制的 HTML，所有字号、颜色、边距和案例样式使用内联 CSS。用户只要文字时，交付紧凑正文，不额外解释排版过程。
